...
(1) EV / Extended-View tables:
      EV_Table_8*.xlsx   -- MRC5 CAM/IMT proteomics
      EV_Table_10*.xlsx  -- 30-model TIS meta-analysis (Anerillas)

(2) Not part of any EV table -- annotation / category-definition files (a curated gene
    list, a MitoCarta3.0 pathway annotation, and Payea et al. 2024 published data), must
    be bundled as their own standalone files alongside this script in the deposit:
      Supplementary_PanelE_Data_1.xlsx
      analMito_Anerillas2026.xlsx
      analMito_Payea2024.xlsx

Note: EV_Table_8 does not carry a 'MC3.0 Mito Pathways' column. That column is only used
to build the 'Mito_gene_expression' category, which is not one of the four categories
actually plotted/correlated in the final EV1O figure, so this has no effect on the
figure's output. To avoid dropping that category outright, this script reuses the
MitoCarta3.0 pathway annotation already loaded from analMito_Anerillas2026.xlsx (the
same universal gene -> pathway mapping used for the other models, not CAM/IMT-specific)
for the CAM/IMT gene universe too.

Put ./data/ (or the files directly next to this script) with all of the above, then run
top to bottom.
...
# ---- to the deposit as their own standalone files (see docstring above) --------
PANELE_PATH = find_file(
    ['Supplementary_PanelE_Data_1.xlsx'],
    'canonical per-pathway gene lists (OXPHOS/FAO/BCAA/NEAA/Sulfur/Methionine '
    'categories) -- NOT an EV table, needs to be added as a standalone deposited file')
...
# Shared MitoCarta3.0 pathway lookup (gene -> pathway string), loaded once here so it
# can be reused for both the 30-model set below and the MRC5/CAM-IMT set above -- see
# the note in this script's docstring: EV_Table_8 itself does not carry this column,
# so the same universal (not model-specific) annotation is reused for it.
...
print('MRC5 Mito_gene_expression genes:', len(ge_genes_mrc5),
      ...
      'universe -- EV_Table_8 has no MitoCarta pathway column of its own. Not used by '
      'the final EV1O plot either way.)')
